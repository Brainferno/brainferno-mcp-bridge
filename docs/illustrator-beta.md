...
That is exactly complementary to `brainferno-mcp-bridge`'s own Illustrator lane (the `ai_*` tools, which
...
Register Adobe's server directly with Claude Code, alongside `brainferno-mcp-bridge`:
...
## Model B — through brainferno-mcp-bridge (the delegate lane)

Give `brainferno-mcp-bridge` the key and it proxies Adobe's server behind three tools, so everything is one
...
BRAINFERNO_MCP_ILLUSTRATOR_KEY=ilst_<your-key>

# …or ~/.brainferno-mcp-bridge/config.json (chmod 600):
...
(accepted / Illustrator not running / refused) and saves it to `~/.brainferno-mcp-bridge/config.json`
...
`BRAINFERNO_MCP_ILLUSTRATOR_URL`), paste the new key, and re-run the installer. If the pasted
