# Spike 1 — Photoshop UXP panel ↔ brainferno-mcp-bridge hub
...
1. Start `brainferno-mcp-bridge` (Claude Code does this; it writes `~/.brainferno-mcp-bridge/bridge.json` with
...
4. In Photoshop the panel appears under **Plugins ▸ brainferno-mcp-bridge bridge (spike)**.
...
- The manifest lists `ws://127.0.0.1:7897` and `ws://localhost:7897` explicitly: UXP needs the
  ...
  change `BRAINFERNO_MCP_BRIDGE_PORT`, change the manifest too and re-load.
